...
```mermaid
flowchart TD
  A["定義目標受眾<br/>角色、經驗、動機"] --> B["設定 2–3 個<br/>具體學習目標"]
  B --> C["反向設計評估方式<br/>如何證明達成目標？"]
  C --> D["設計課程內容<br/>與活動（支撐目標）"]
  D --> E["規劃應對<br/>常見挑戰的策略"]
  E --> F["同儕互評<br/>與修改完善"]
```
...
```mermaid
flowchart TD
  L1["🔲 知識層<br/>能說出 4Ds 定義"] --> L2
  L2["🔳 理解層<br/>能解釋各 D 的用法"] --> L3
  L3["⬛ 應用層<br/>在真實任務中運用 4Ds<br/>⭐ 核心目標"] --> L4
  L4["🏆 反思層<br/>反思 AI 使用習慣並持續調整<br/>⭐ 最高層次"]
```
