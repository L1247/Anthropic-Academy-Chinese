...
完成 NLM 頁面產出後，進行主課程頁的大綱搬遷。**核心原則：大綱內容隨 NLM 頁面建立而移出主頁。**
...
將 `<details>` 區塊的核心內容融入 NLM 頁面的 `## 📋 課程概覽` 段落（或 `## 📖 課程大綱深度說明` 獨立節），確保原先在主頁可見的深度說明在 NLM 頁中保存完整。

**9.1.C 主課程頁：完全移除該課的 H3 大綱區塊**

將主課程頁中原有的 `### 第 NN 課：` H3 區塊（包含其下的描述文字、`<details>` 內容、任何精簡說明與連結）**整體刪除**，不留任何殘餘。

> ⚠️ **強制完全移除**：一旦 NLM 頁面建立，對應課號的 H3 區塊必須從主課程頁**完全消失**（參考第 01、02 課的已完成狀態）。
> 課程導覽由 sidebar 的 NLM 連結承擔；主頁大綱只保留尚未建立 NLM 頁的課程。

**注意**：
- 刪除後確保前後空行整齊，下一個 H3 或元件緊接其後
- 僅移除對應課號的區塊，不動其他課號
- 若刪除後主頁課程大綱區段全空（全部課程已完成），保留 `## 📋 課程大綱` 標題即可，或視情況移除整個空白節
